...
Universal test definitions (``mcp-suite.yaml`` / ``cases:`` in config) are
compiled into :class:`~mcp_test_harness.discovery.HarnessCase` objects so the
existing scheduler, fixtures, and reporters run unchanged.  Language adapters
(TypeScript, Java, Go) invoke the same engine via the ``mcp-test`` CLI.
"""
